 ...
 */
export const useFileUpload = (userAddress?: string) => {
  const [progress, setProgress] = useState(0);
  const [status, setStatus] = useState("");
  const [uploadedInfo, setUploadedInfo] = useState<UploadedInfo | null>(null);
  const { synapse } = useSynapse();
  const { triggerConfetti } = useConfetti();
  // Use the passed userAddress or fallback to World App context
  const address = userAddress || (typeof window !== 'undefined' && (window as any).MiniKit?.walletAddress) || "0x0000000000000000000000000000000000000000";
